...
function isCreditCardNumber(ccn) {
  const ccnString = ccn.toString().replace(/\s/g, '');
  if (!/^\d+$/.test(ccnString)) {
    return false;
  }
  const ccnArray = Array.from(ccnString, Number);
  ccnArray.reverse();
  ...
  const sum = ccnArray.reduce((acc, curr) => acc + curr, 0);
  return sum % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < minLength; i += 1) {
    const firstDirectory = directories[0][i];
    if (directories.every((dir) => dir[i] === firstDirectory)) {
      commonPath += `${firstDirectory}/`;
    } else {
      break;
    }
  }
